...
function AppLogo({
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      {/* Show image if src provided, otherwise show icon if iconName is provided and no text, or if explicitly requested */}
      {src ? (
      ...
      {text && <span className="text-2xl font-fraunces italic font-medium tracking-tight">{text}</span>}
    </div>
  );
}
